...
prompt = (
    ...
    "규칙:\n"
    "- '수집 실패' 항목은 해석하지 말고 실패했다고만 적을 것\n"
    "- 매수/매도 추천 금지. 관찰된 사실과 그 함의만 서술\n"
    ...
    "SMH(반도체), XLK(기술), IGV(소프트웨어), ITA(방산), ARKX(우주), LIT(배터리), GRID(전력망·인프라), XLU(유틸리티), NVDA(엔비디아), TSM(TSMC), ETN(이튼), PWR(콰나서비스), CEG(콘스텔레이션에너지)\n"
    "- 머리말이나 마무리 인사 없이 위 형식만 출력\n\n"
